...
- Repo secret `FORK_SYNC_TOKEN`:PAT(contents write),fork-sync checkout/push 用它而非默认 `GITHUB_TOKEN`——默认 token 的推送不触发其他 workflow,会静默漏掉 `v*-fork.*` 的镜像构建和 main push 的 test.yml。
