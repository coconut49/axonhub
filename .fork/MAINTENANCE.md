...
- `main` 永远等于「某个上游 release tag + fork patch 栈」。base tag 不单独记录,随时可推导(与上游的 merge-base,且必须恰好是一个上游 release tag;若推导失败说明 main 基于未打 tag 的上游 commit,属异常状态):

  ```sh
  git describe --tags --exact-match --exclude '*-fork.*' "$(git merge-base main upstream/unstable)"
  ```
...
- 上游 tag 版本序:`-alpha` < `-beta` < 正式版,按版本排序时(如 fork-sync 检测最新上游 tag)必须带 `-c versionsort.suffix=-alpha -c versionsort.suffix=-beta`,否则 `v1.0.0-beta5` 会排在 `v1.0.0` 之后。
...
3. 跑与上游 CI 相同的后端测试(`make test-backend-all`,根模块 + `llm/` 两个 Go module),并构建前端(`pnpm vite build`,node/pnpm 版本对齐 Dockerfile)——上游 CI 不测前端,不在此把关会推迟到镜像构建才暴露。
4. 通过后 `git push --force-with-lease origin main`,打 `v*-fork.N` tag 并推送;tag push 事件触发 `fork-image.yml` 构建镜像。推送走 `FORK_SYNC_TOKEN`(见「环境事实」)。
...
5. 需要出镜像时打 tag 并推送:
   ...
   ```sh
   base=$(git describe --tags --exact-match --exclude '*-fork.*' "$(git merge-base main upstream/unstable)")
...
- 由 `.github/workflows/fork-image.yml` 在 `v*-fork.*` tag push 时构建(本地手动推或 fork-sync 用 PAT 推均产生 push 事件);沿用上游机制,把 tag 写入 `internal/build/VERSION` 后 docker build(amd64/arm64 各自原生 runner 构建,再合 manifest)。
...
- Repo secret `FORK_SYNC_TOKEN`:fine-grained PAT,仅授权本仓库,权限 Contents/Workflows/Issues 均 Read-Write,供 fork-sync 推送与开 issue。默认 `GITHUB_TOKEN` 的推送不产生 push 事件(会静默漏掉镜像构建与 test.yml),且不能推送含 workflow 文件变更的 commit。注意 PAT 有效期,过期后 fork-sync 会连续失败且无法开 issue 通知。
- 仓库 Issues 功能须保持启用,fork-sync 失败通知依赖 `gh issue create`。
