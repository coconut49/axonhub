...
5. 完成后自查:`git log <base>..HEAD --oneline`(base = 最高的、是 HEAD 祖先的非 fork tag)应与 `.fork/PATCHES.md` 条目一一对应;编译最容易受影响的包确认无语法级错误——注意本仓库是双 Go module,根目录 `go build ./...` 与 `cd llm && go build ./...` 都要跑。
