...
func TestRequestService_PreviousChannelByThreadUsesLatestTraceRequest(t *testing.T) {
	cacheConfig := xcache.Config{Mode: xcache.ModeMemory}
	ctx, client, service := newRequestStickyTestService(t, cacheConfig, cacheConfig)
	threadEntity, firstTrace, firstChannel := createRequestStickyScope(t, ctx, client)

	secondChannel, err := client.Channel.Create().
		SetName("request-sticky-channel-newer").
		SetType(channel.TypeOpenai).
		SetBaseURL("https://example.com/v1").
		SetCredentials(objects.ChannelCredentials{APIKey: "test-key"}).
		SetSupportedModels([]string{"gpt-4"}).
	...
	secondTrace, err := client.Trace.Create().
		SetProjectID(firstTrace.ProjectID).
		SetThreadID(threadEntity.ID).
		SetTraceID("request-sticky-trace-newer").
		Save(ctx)
	require.NoError(t, err)

	now := time.Now()
	createStickyRequest(t, ctx, client, firstTrace.ID, firstChannel.ID, request.StatusProcessing, now.Add(-time.Minute))
	createStickyRequest(t, ctx, client, secondTrace.ID, secondChannel.ID, request.StatusProcessing, now)
